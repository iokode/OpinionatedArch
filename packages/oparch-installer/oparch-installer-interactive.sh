#!/bin/sh
#
# What goes on PATH. The interactive installer's host loads the BAML runtime as
# a shared library rather than carrying it, so it has to be told where the copy
# on this system is; and BAML_LIBRARY_DISABLE_DOWNLOAD turns a library that is
# not there into a failure of the run rather than a silent fetch, which is what
...
for argument in "$@"; do
    if [ "$argument" = "--assets" ]; then
        exec "${OPARCH_LIB}/oparch-installer-interactive" "$@"
    fi
done

exec "${OPARCH_LIB}/oparch-installer-interactive" --assets "${OPARCH_ASSETS}" "$@"
