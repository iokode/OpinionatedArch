...
`oparch-installer` runs from a live environment and has no use on a machine that is installed, and every other operational tool of this project is installed on the machine afterwards, with pacman. The four this project publishes are the renderer and the dotfiles tool, which the installation runs, the assets the renderer reads, and the keyring that lets them be updated from where they came from. What that is, and why the repository sits above the official ones on the installed system, is [Package Repository](../../../decisions/016-package-repository.md); the installation writes it into the target as the published repository and never as the one the medium carries, which will not be there once the machine is running.
