...
A system snapshot takes the boot artifacts of that moment with it, as [Snapshots](../../../decisions/004-snapshots.md) requires: the tool hashes them, stores the set under the name its hash gives it when that set is not there already, and records which set belongs to the snapshot it has just made, as [Boot Artifacts Table Format](002-boot-table-format.md) specifies. A set of boot artifacts is deleted with the last snapshot that points at it.
