...
[Pre-Boot Ownership Message](../../../decisions/009-preboot-ownership-message.md) left the question open and [Remaining](../../../state/001-remaining.md) recorded it: whether the rendered message may be decorated by something supplied the way a template is, and what such a thing may control.
...
This replaces the fixed limit of four that [Pre-Boot Ownership Message](../../../decisions/009-preboot-ownership-message.md) set.
...
- The theme is validated with the package rather than at the moment each value is used, because a theme that cannot be read is a configuration error, and a configuration error found halfway through leaves a partially installed disk. This is the rule [Pre-Boot Ownership Message](../../../decisions/009-preboot-ownership-message.md) already sets for template packages.
...
- Return-message readability still has to be validated on the real display resolutions used by the target machines, as [Pre-Boot Ownership Message](../../../decisions/009-preboot-ownership-message.md) already requires. A theme makes that easier to correct and does not make it unnecessary.
