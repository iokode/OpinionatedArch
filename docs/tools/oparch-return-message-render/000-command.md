...
It reads the values that fill the template from `/etc/opinionatedarch/return-message.yaml`, written during installation and editable afterwards, and defined in [Return Message Values Format](002-values-format.md):
...
Beside them it writes `opinionatedarch.script`, the script the splash runs: the values of the theme's `screen` section, written as literals, followed by the body the project carries in its assets. Why the splash is handed them rather than reading them is in [Return Message Theme Format](003-theme-format.md).
...
How many languages may be selected, and how they are arranged, is the theme's: it declares an arrangement for each number it accepts. What that arrangement should aim for is argued in [Pre-Boot Ownership Message](../../decisions/010-preboot-ownership-message.md) — a composition closer to the shape of a screen than to a column, because a tall composition on a wide screen is limited by its height and ends up small.
...
Building an image, rather than having the boot splash draw text, is decided in [Pre-Boot Ownership Message](../../decisions/010-preboot-ownership-message.md). The short of it: the splash draws with one font and no fallback, so any writing system that font does not cover renders as blanks, and it cannot justify, wrap or emphasise.
...
- `--template-package <path>`: Optional. Directory holding the template package to compose, as defined in [Return Message Template Package Format](001-template-package-format.md). Default: the project's own, inside `--assets`.
- `--theme <path>`: Optional. Directory holding the theme to compose it with, as defined in [Return Message Theme Format](003-theme-format.md). Default: the project's own, inside `--assets`.
