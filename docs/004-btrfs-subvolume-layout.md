...
- `@snapshots`: stores all snapshot data in one dedicated container with structured paths (`system` and `<login-user>`) so snapshot storage stays centralized without mixing system and user histories semantically; if omitted, snapshot storage layout becomes fragmented or ambiguous.
...
5. Provision each login user with a dedicated home subvolume and initialize `/snapshots/system` plus `/snapshots/<login-user>` paths.
...
- User provisioning must include home-subvolume creation and per-user snapshot-path creation under `/snapshots` for install-time and post-install users.
