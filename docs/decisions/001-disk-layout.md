...
- `@snapshots` is the snapshot-storage subvolume. The snapshots inside it are subvolumes too, each named `@<unix-seconds>` after the moment it was taken, at these paths:
  ...
  - `@snapshots/boot` holds one directory per distinct set of boot artifacts, named by the hash of its contents, and the table that pairs each system snapshot with the set that belongs to it, as [Snapshots](004-snapshots.md) decides and [Boot Artifacts Table Format](../tools/snapshot/system-create/002-boot-table-format.md) specifies. These are files rather than subvolumes: what they copy lives on the EFI system partition, outside Btrfs.
  - `@snapshots/labels.json` pairs each manual snapshot with its justification, as [Snapshot Labels File Format](../tools/snapshot/system-create/001-labels-file-format.md) specifies. It is a file rather than a subvolume.
  - `@snapshots/lock` is the file whose lock is held while any of the three above is written, as [Snapshot Storage Lock](../tools/snapshot/system-create/003-storage-lock.md) specifies.
