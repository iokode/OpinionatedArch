...
The BAML runtime library travels in a package of its own, `oparch-baml-runtime`, which `oparch-installer` depends on, and `oparch-installer-interactive` is pointed at the copy on the image. It is never downloaded: a library that is not there is a failure of the run and not a fetch.
...
- The image an image replaces is kept, and that is for the downloads already under way. An address answers with whichever image is current, but a download is not an instant: an image is gigabytes, and somebody who started one is reading that file for minutes afterwards. Deleting it the moment its successor is up would end their transfer part way through, for no reason other than that a newer one exists. Keeping it until the day after costs the space of one image and means nobody's download is interrupted by a publication.
- Images are not archived, and the one kept is not a version anybody can choose. There is one address for an image and it answers with the current one; nothing publishes the name an older one was given, so the one held back is reachable only by whoever was already downloading it. What that costs is said rather than argued away: a machine was installed from an image, and which one is part of what that machine is, so an installation cannot be repeated from the medium it was made with once that medium is gone. What survives of it is its name and its checksum in whatever recorded them, which says which image a machine came from without being able to produce it.
