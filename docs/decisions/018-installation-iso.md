...
- Two are kept, and what that costs is said rather than argued away. A machine was installed from one of them, and which one is part of what that machine is; an image that has been deleted is an installation that cannot be repeated from the medium it was made with. What survives of an older one is its name and its checksum in whatever recorded them, which says which image a machine came from without being able to produce it. Two is what an image this size allows with room to spare — carrying the packages it installs is what makes it this size, and the alternative to that was an image that could not install without a network. The month's image and the one before it is also what anyone actually reaches for: the current one, and the one to fall back to when it turns out to be wrong.
