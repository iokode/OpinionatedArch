...
The keymap used at unlock is the machine's own, as [Localization and Time](005-localization-and-time.md) decides it. The `keymap` hook reads it from `/etc/vconsole.conf` and puts it into the image.
...
- No hook prepares Plymouth for text because Plymouth draws none: everything the unlock screen shows is an image, as decided in [Pre-Boot Ownership Message](009-preboot-ownership-message.md). A hook setting the process locale existed to make its label renderer decode text correctly, and there is no text left for it to decode.
