...
- No hook prepares Plymouth for text because Plymouth draws none: everything the unlock screen shows is an image, as decided in [Pre-Boot Ownership Message](010-preboot-ownership-message.md). A hook setting the process locale existed to make its label renderer decode text correctly, and there is no text left for it to decode.
