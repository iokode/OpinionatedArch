...
The installed system keeps two separate paths: `/usr/oparch` for this installer repository, and `/dotfiles` for active shared runtime configuration.
...
- Separating `/usr/oparch` from `/dotfiles` is required because installer maintenance and runtime config maintenance are different concerns; if both are mixed into one path, updates and troubleshooting cross-contaminate.
- Copying this repository into `/usr/oparch` with `.git` is required because post-install review needs exact script sources plus history; if `.git` is missing, traceability and rollback analysis lose commit context.
