...
`Target` is what names which root is meant, and whether a command aimed at it has to be run inside it. It lives in the shared code, with the `Shell` and `Files` ports it is used with, for the reasons `002-repository-layout.md` gives.
