...
`Target` is what names which root is meant, and whether a command aimed at it has to be run inside it. It lives in the shared code, with the `Shell` and `Files` ports it is used with, for the reasons [Repository Layout](002-repository-layout.md) gives.
...
Nothing else, and nothing else is expected to. A tool that has to change another system is entered rather than aimed at, as [Acting on Another System](004-acting-on-another-system.md) decides, so it works on the machine it runs on by construction: `oparch-dotfiles-sync` and `oparch-return-message-render` call the shell directly, which is the same thing a local target would do. That `oparch-dotfiles-sync` can be told which hostname and which accounts to plan for does not change it — being told about a machine is not being aimed at one, and what it writes it still writes into its own root. That is worth stating plainly rather than leaving the abstraction looking more used than it is: it is here because a second kind of caller is expected — anything that repairs or inspects a system it is not running on — and because the alternative was for the installer to keep owning it, where a second caller would have had to reach into a tool to get at it.
