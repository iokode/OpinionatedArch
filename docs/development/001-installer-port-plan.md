...
- `baml/utils/` — the generic BAML, namespace `root.common`: the `Shell` and `Files` ports, the host adapter for commands, their recording doubles, their implementations over `baml.sys` and `baml.fs`, and helpers for text, paths and YAML.
...
- Layout and the reason for it: `005-baml-repository-layout.md`.
