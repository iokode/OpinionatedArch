...
The harness is `tests/e2e/run.sh`. It is the only test that lives outside the source of a tool, for the reason `002-repository-layout.md` gives: it belongs to no project and tests all of them at once, where a unit test is a `test` block compiled with the code it exercises.
...
Its guest runs with `-nographic`, so there is no display for a splash. Plymouth falls back to the text prompt `../decisions/010-preboot-ownership-message.md` requires, which is worth knowing works, but the harness therefore never draws the composed message and never runs the script the renderer writes.
