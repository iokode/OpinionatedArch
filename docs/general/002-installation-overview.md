...
Prompts cover the target disk and install mode, microcode and GPU driver, swap sizes, login usernames, the shared secret, keymap, timezone, hostname, an optional public dotfiles repository, and the optional pre-boot return message with its ownership fields, languages, and logo.
