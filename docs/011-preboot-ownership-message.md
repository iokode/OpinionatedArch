...
3. If `yes`, add a Plymouth theme under project assets for pre-boot unlock.
4. Prompt for owner name, phone, email, and return address values.
5. Add four localized text blocks with labels (`ENG`, `CAT`, `ESP`, `FRA`) and the same owner/contact data in each block.
6. Add PNG flag assets and place them next to each language block.
7. Prompt whether to include a logo (`yes/no`).
8. If `yes`, prompt for `logo_url` and attempt download during installation.
9. If download fails, prompt for explicit choice: retry with a new URL or continue without logo.
10. If logo download succeeds, copy it into Plymouth theme assets used for initramfs generation.
11. Arrange the layout as a 2x2 grid with clear separators.
