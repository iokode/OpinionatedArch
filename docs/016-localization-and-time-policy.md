...
- Asking for keymap is required because keyboard layout depends on the physical keyboard used by each machine; if keymap is fixed globally, installations on different layouts (for example AZERTY vs QWERTY variants) will produce wrong key mapping and problematic unlock/input behavior.
