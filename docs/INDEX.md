...
`oparch-installer`, `oparch-installer-interactive`, `oparch-return-message-render`, `oparch-dotfiles-sync`, `oparch-password-rotate` and `oparch-password-rotate-interactive` are written. The rest are specified and not implemented, which [Remaining](state/001-remaining.md) carries as the first work.
