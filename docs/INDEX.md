...
`oparch-installer`, `oparch-installer-interactive`, `oparch-return-message-render`, `oparch-dotfiles-sync` and `oparch-password-rotate` are written. The rest are specified and not implemented, which [Remaining](state/001-remaining.md) carries as the first work.
